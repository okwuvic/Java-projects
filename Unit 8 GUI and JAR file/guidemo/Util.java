...
public class Util {
	...
	/**
	 * Play a sound that is stored as a resource file. If the resource can't be
	 * located or can't be played, no sound is played, and no exception is thrown.
	 * 
	 * @param pathToResource the path to the resource.
	 */
	...
	/**
	 * Load an AudioClip from a resource file. The clip can be played by calling its
	 * play() method.
	 * 
	 * @param pathToResource the path to the resource.
	...
	/**
	 * Create a cursor from an image resource file, with hot point at the upper left
	 * corner (0,0).
	 * 
	 * @param pathToResource the path to the resource.
	 * @return the cursor or, if the resource can't be loaded, the default cursor.
	 */
	...
	/**
	 * Create a cursor from a resource file, with hot point at (hotSpotX, hotSpotY).
	 * 
	 * @param pathToResource the path to the resource.
	 * @return a cursor that will show the image.
	 */
	public static Cursor createImageCursor(String pathToResource, int hotSpotX, int hotSpotY) {
		...
		if (img == null)
			...
			return Toolkit.getDefaultToolkit().createCustomCursor(img, new Point(hotSpotX, hotSpotY), pathToResource);
	}
	...
	public static Cursor createImageCursor(Image image, int hotSpotX, int hotSpotY) {
		return Toolkit.getDefaultToolkit().createCustomCursor(image, new Point(hotSpotX, hotSpotY), null);
	}
